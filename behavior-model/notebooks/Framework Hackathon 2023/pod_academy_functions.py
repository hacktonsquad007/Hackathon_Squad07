...
print('-----------------------------------------------------')
print('Atualização: 1.0.1')
print('Aluno: Edgar Cabral')
...
def pod_custom_fillna_prod(df_orig, means):
    import numpy as np
    import pandas as pd
...
def pod_custom_lowfreq(df):
    ...
    for col in categorical_cols:
        ...
        category_freq = df[col].value_counts()

        # Obter o índice com a menor frequência e a frequência correspondente
        indice_menor_frequencia = category_freq.idxmin()
        menor_frequencia = category_freq.min()
    
        # Armazenar os resultados na DataFrame 'resultados'
        resultados_lista.append({
            'Coluna': col,
            'Indice': indice_menor_frequencia,
            'Frequencia': menor_frequencia})
    
    # Criar uma DataFrame a partir da lista de resultados
    resultados = pd.DataFrame(resultados_lista)
    resultados = resultados.sort_values(by='Frequencia')
